...
<div class="main-dashboard">
    <div class="alert alert-primary">
        <b>به پنل کتابداری خوش آمدید</b>
        <br>
        <br>
        <?php
        $sql = "SELECT nid,ntitle FROM notification WHERE (nstatus=3 || nstatus=5 || (nstatus=6 && reciver=$scode)) ORDER BY nid DESC";
    ...
    <div class="row">
        <div class="col-12 col-lg-2 box-report bg-green">
                تیکت مشاهده نشده
                <i class="fa fa-ticket fa-big"></i>
                <br>
                <small>
                    <?php
                    $sql = "SELECT COUNT(tid) AS answer FROM ticket WHERE tstatus=0";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        // output data of each row
                        while($row = $result->fetch_assoc())
            ...
            <div class="col-12 col-lg-2 box-report bg-blue">
                کاربران غیرفعال
                <i class="fa fa-ticket fa-big"></i>
                <br>
                <small>
                    <?php
                    $sql = "SELECT COUNT(sid) AS answer FROM student WHERE status=0";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        // output data of each row
                        while($row = $result->fetch_assoc())
            ...
            <div class="col-12 col-lg-2 box-report bg-gray">
                بلاگ غیرفعال
                <i class="fa fa-ticket fa-big"></i>
                <br>
                <small>
                    <?php
                    $sql = "SELECT COUNT(blid) AS answer FROM blog WHERE status=0";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        // output data of each row
                        while($row = $result->fetch_assoc())
            ...
            <div class="col-12 col-lg-2 box-report bg-magenta">
                رزرو مشاهده نشده
                <i class="fa fa-ticket fa-big"></i>
                <br>
                <small>
                    <?php
                    $sql = "SELECT COUNT(rrid) AS answer FROM request_reserve WHERE status=0";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        // output data of each row
                        while($row = $result->fetch_assoc())
            ...
            <div class="col-12 col-lg-2 box-report bg-yellow">
                درخواست مشاهده نشده
                <i class="fa fa-ticket fa-big"></i>
                <br>
                <small>
                    <?php
                    $sql = "SELECT COUNT(rid) AS answer FROM request WHERE rstatus=0";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        // output data of each row
                        while($row = $result->fetch_assoc())
            ...
            <div class="col-12 col-lg-2 box-report bg-dark">
                اعلان
                <i class="fa fa-ticket fa-big"></i>
                <br>
                <small>
                    <?php
                    $sql = "SELECT COUNT(nid) AS answer FROM notification WHERE nstatus=0";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        // output data of each row
                        while($row = $result->fetch_assoc())
        ...
        <div class="box-dashboard col-11 col-lg-4">
            ...
            <div class="row">
                <?php
                $sql = "SELECT * FROM student WHERE status=4 && scode !=$scode";
                $result = $conn->query($sql);
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        <div class="col-8" style="padding: 5px 27px;">
                            <?php
